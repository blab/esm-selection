...
A="ESM_1386_Full.fasta"
B="H3N2_Dataset_Full.fasta"
C="pan_flu_1386_Full.fasta"
...
pickAB=${half:-0}; [ "$pickAB" -gt 0 ] || pickAB=$(min2 "$nA" "$nB")
pickBC=${half:-0}; [ "$pickBC" -gt 0 ] || pickBC=$(min2 "$nB" "$nC")
pickAC=${half:-0}; [ "$pickAC" -gt 0 ] || pickAC=$(min2 "$nA" "$nC")

if [ "$TOTAL_TRIPLE" -gt 0 ]; then third=$((TOTAL_TRIPLE/3)); else third=0; fi
pickABC=$third; [ "$pickABC" -gt 0 ] || pickABC=$(min3 "$nA" "$nB" "$nC")

# AB 50/50
seqkit sample -s "$((SEED+1))" -n "$pickAB" "$A" > __A_AB.fasta
seqkit sample -s "$((SEED+2))" -n "$pickAB" "$B" > __B_AB.fasta
cat __A_AB.fasta __B_AB.fasta | seqkit shuffle -s "$((SEED+3))" > mix_AB_50_ESM_1386_Full_50_H3N2_Dataset_Full.fasta

# BC 50/50
seqkit sample -s "$((SEED+4))" -n "$pickBC" "$B" > __B_BC.fasta
seqkit sample -s "$((SEED+5))" -n "$pickBC" "$C" > __C_BC.fasta
cat __B_BC.fasta __C_BC.fasta | seqkit shuffle -s "$((SEED+6))" > mix_BC_50_H3N2_Dataset_Full_50_pan_flu_1386_Full.fasta

# AC 50/50
seqkit sample -s "$((SEED+7))" -n "$pickAC" "$A" > __A_AC.fasta
seqkit sample -s "$((SEED+8))" -n "$pickAC" "$C" > __C_AC.fasta
cat __A_AC.fasta __C_AC.fasta | seqkit shuffle -s "$((SEED+9))" > mix_AC_50_ESM_1386_Full_50_pan_flu_1386_Full.fasta
...
cat __A_ABC.fasta __B_ABC.fasta __C_ABC.fasta | seqkit shuffle -s "$((SEED+13))" > mix_ABC_33_ESM_1386_Full_33_H3N2_Dataset_Full_33_pan_flu_1386_Full.fasta
